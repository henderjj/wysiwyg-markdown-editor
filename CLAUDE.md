...
**In-document links and the shell plugin:** TipTap's Link renders every anchor with `target="_blank"`, and in the Tauri build `tauri-plugin-shell` injects a click listener on `<body>` that hands any `_blank` `http(s)` link to the system browser. A bare `#fragment` resolves to `http://tauri.localhost/#fragment`, so it opened a browser tab (issue #52). The `InternalLinkNavigation` extension in `Editor.tsx` handles clicks on `#` links on the editor's own DOM, calls `stopPropagation()` so the event never reaches `<body>`, and jumps to the heading whose GitHub-style slug matches (`src/lib/headingAnchors.ts`). Keep the `stopPropagation()`: `preventDefault()` alone does not stop the shell listener. The plain web build has no such listener, so browser testing needs a copy of it injected to reproduce the bug.
