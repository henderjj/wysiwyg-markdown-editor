...
**In-document links and the shell plugin:** TipTap's Link renders every anchor with `target="_blank"`, and in the Tauri build `tauri-plugin-shell` injects a click listener on `<body>` that hands any `_blank` `http(s)` link to the system browser. A bare `#fragment` resolves to `http://tauri.localhost/#fragment`, so it opened a browser tab (issue #52). The `LinkNavigation` extension in `Editor.tsx` handles *every* link click on the editor's own DOM and calls `stopPropagation()` so none reaches `<body>` — that is what makes links Ctrl+Click (Cmd+Click) to follow, leaving a plain click free to place the caret for editing link text. On Ctrl+Click a `#` link jumps to the heading whose GitHub-style slug matches (`src/lib/headingAnchors.ts`) and anything else goes to `openExternalUrl()` in `tauri.ts` (shell plugin `open`, covered by the existing `shell:allow-open` permission; `window.open` on the web; http(s)/mailto/tel only). Keep the `stopPropagation()`: `preventDefault()` alone does not stop the shell listener, and plain clicks would start opening the browser again. The plain web build has no such listener, so browser testing needs a copy of it injected to reproduce the bug.

**Link dialog and hover tooltip** (`LinkDialog.tsx`, `LinkMenu` in `MenuBar.tsx`, `src/lib/linkEditing.ts`): the 🔗 button captures a `LinkTarget` from the selection when it opens — edit mode exactly when `isMarkActive(state, link)` is true, the same test that highlights the button, so the caret just past a link's end (not inside it, since Link is non-inclusive) inserts rather than edits. `applyLink()` only re-marks the range when the display text is unchanged, which keeps bold/code inside a link; changed text is replaced with a single text node carrying the first character's marks. Text is locked when the range spans blocks or holds inline atoms (images, 💬 comments), because replacing it would delete them. The hover tooltip is the styled `[data-link-tip]:hover::after` rule in `index.css`, and its text comes from an inline **decoration** in `LinkNavigation`'s plugin state that sets `data-link-tip` on the text inside each link. Decorations never reach `getHTML()`, whereas an attribute in Link's `HTMLAttributes` would be saved. Don't switch it to `title`: the native tooltip then shows as a second box alongside the styled one.
...
**CriticMarkup review comments** (`src/extensions/critic-markup/`): `{==text==}{>>comment<<}` is a `criticHighlight` mark with a `comment` attribute, a lone `{==text==}` is the same mark with `comment: null`, and a lone `{>>comment<<}` is a `criticComment` inline atom node. The parser pass in `parseInline()` runs after the escape and code-span stashes (so `\{==` and code stay literal) and before emphasis, and stashes the comment body so emphasis/link regexes can't rewrite it. Three non-obvious constraints: (1) the mark has `priority: 1100`, above Link's 1000, so it is the *outermost* mark — otherwise a comment spanning bold or link text serializes as several `<mark>`s and exports as several comments; (2) the atom node renders a literal `💬` text child because Turndown's blank rule drops text-less elements *before* any custom rule runs, which would silently lose the comment on save; (3) comment bodies are plain text in an attribute, so `turndownEscape` never sees them — `escapeCriticComment()` escapes `\`, `` ` `` and `<<}` itself (backticks so no code span can form inside a body, which would otherwise make backslashes grow on every round-trip). `turndownEscape` escapes a `{` followed by `==`/`>>` and `=` followed by `=}` in text nodes, so literal CriticMarkup typed by the user stays literal. The stored comment string is never rewritten; the `[Author YYYY-MM-DD HH:MM]: ` prefix is parsed for display only (`commentMeta.ts`). Author defaults to the OS display name via the `get_user_real_name` Tauri command (`whoami` crate). The UI is one `CommentBubble` (view / add / edit), positioned by `@floating-ui/dom` against the comment's *DOM element* (or a `posToDOMRect` virtual element for a range being commented on) — never a rect snapshot, which is what made the old popover jump between hover and caret positions and detach on scroll. The bubble starts `visibility: hidden` until its first position is computed, and a hidden input silently refuses focus, so its input is focused in an effect once placed; `autoFocus` does not work there. While composing, focus is in the bubble, so the target range is drawn by a decoration (`setPendingComment`), not the native selection.
